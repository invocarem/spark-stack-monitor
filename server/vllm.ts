...
const CHAT_TIMEOUT_MS = Number(process.env.VLLM_CHAT_TIMEOUT_MS ?? "120000");
...
/** POST /v1/chat/completions on the vLLM server (OpenAI-compatible). */
export async function forwardVllmChatCompletions(
  body: unknown,
  ...
  const url = new URL("/v1/chat/completions", `${getVllmBaseUrl()}/`);
  const payload = {
...
const BENCHMARK_MAX_REQUESTS = Number(process.env.VLLM_BENCHMARK_MAX_REQUESTS ?? "200");
const BENCHMARK_MAX_CONCURRENCY = Number(process.env.VLLM_BENCHMARK_MAX_CONCURRENCY ?? "32");
/** Max chars of assistant text returned as `sampleContent` (request index 0). */
const BENCHMARK_SAMPLE_MAX_CHARS = Number(process.env.VLLM_BENCHMARK_SAMPLE_MAX_CHARS ?? "32000");
 ...
 * vLLM/OpenAI-compatible responses may use string or array `content`.
 */
...
export async function runVllmBenchmark(
  body: unknown,
): Promise<VllmBenchmarkOk | VllmBenchmarkErr> {
  ...
  async function oneShot(): Promise<void> {
    while (true) {
      ...
      const t0 = Date.now();
      const result = await forwardVllmChatCompletions(payload);
